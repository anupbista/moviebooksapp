...
export default function Navbars() {
  ...
  const fetchGenre = async () => {
    const res = await Axios.get(`https://ent-api-dev.herokuapp.com/api/v1/genre`)
    setGenre(res.data);
  } 
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="mr-auto nav-link dropdown-toggle">
              <NavDropdown title="Movies" >
                  <Nav.Link className="nav-link" as={Link} to="/movies" >All</Nav.Link>
                  <Nav.Link className="nav-link" as={Link} to="/movies/popular" >Popular</Nav.Link>
                  <Nav.Link className="nav-link" as={Link} to="/movies/upcoming" >Upcoming</Nav.Link>
                  <Nav.Link className="nav-link" as={Link} to="/movies/toprated" >Top Rated</Nav.Link>
                  <NavDropdown title="Genre" >
                  {genre.map(function (gen) {
                    return (
                    <Nav.Link className="nav-link" as={Link} key={gen.id} to={'/movies/genre/' + gen.id } >{gen.name}</Nav.Link>
                    );
                })}
              </NavDropdown>
              </NavDropdown>
              <NavDropdown title="Books" >
                <NavDropdown.Item href="#popular">All</NavDropdown.Item>
